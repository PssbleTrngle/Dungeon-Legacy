...
public class StructureLoader extends ReloadListener<List<LoadedStructure>> {
     ...
     */
    @Override
    protected List<LoadedStructure> prepare(IResourceManager manager, IProfiler profiler) {
    ...
    @Override
    protected void apply(List<LoadedStructure> list, IResourceManager manager, IProfiler profiler) {
     ...
     */
    private static Optional<LoadedStructure> load(IResourceManager manager, ResourceLocation path) {
        try {
            ...
            return manager.getAllResources(path).stream().map(resource -> {
                try {
                    ...
                    return Optional.<LoadedStructure>empty();
                }
            })
            .filter(Optional::isPresent)
            .map(Optional::get)
     ...
     * @return A LoadedStructure instance with the specified metadata reading from the given IResource
     */
    private static LoadedStructure readStructure(IResource resource, StructureMetadata meta) throws IOException {
        ...
        LoadedStructure structure = new LoadedStructure(meta);
