...
public class DungeonMod {
    ...
    public static <B extends Block> RegistryObject<B> registerBlock(String name, Supplier<B> sup) {
        RegistryObject<B> block = BLOCKS.register(name, sup);
    ...
    public static final ItemGroup GROUP = new ItemGroup("dungeon") {
        ...
        public ItemStack createIcon() {
            return new ItemStack(TemplateType.WALL.getBlock(), 1);
        }
    };
    ...
    public static void registerModels(ModelRegistryEvent evt) {
        ...
        GrenadeEntity.TYPE.ifPresent(type -> RenderingRegistry.registerEntityRenderingHandler(type, manager -> new SpriteRenderer<>(manager, i)));
    }

    private void clientSetup(final FMLClientSetupEvent event) {
        ClientRegistry.bindTileEntityRenderer(MetadataTile.TYPE.get(), MetadataTESR::new);
        ClientRegistry.bindTileEntityRenderer(ChestTile.TYPE.get(), ChestTESR::new);

        ObeliskBlock.OBELISK.ifPresent(block -> RenderTypeLookup.setRenderLayer(block, RenderType.getCutout()));
    }

    @OnlyIn(Dist.CLIENT)
    @SubscribeEvent
    public static void blockColors(ColorHandlerEvent.Block event) {
        ObeliskBlock.OBELISK.ifPresent(block -> event.getBlockColors().register((s, w, p, i) -> {
            if (i != 1) return -1;
    ...
    public static void itemColors(ColorHandlerEvent.Item event) {
        ScrollItem.SCROLL.ifPresent(item -> event.getItemColors().register((s, i) -> i == 1 ? ScrollItem.getSpell(s).getColor() : -1, item));
    }
    ...
    public void onServerWillStart(final FMLServerAboutToStartEvent event) {
        ...
        StructureType.HALLWAY.ifPresent(type -> Structures.register(new HallwayMaze(), type));
